(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('myphotogramApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-myphotogramApp-error');
                var entityKey = httpResponse.headers('X-myphotogramApp-params');
                if (errorHeader) {
